...
export function generateTemplateHTML(
  template: string,
  data: ResumeData,
  previewMode = false // NEW: preview-only tweaks
): string {
  ...
  const baseStyles = `
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    ${previewMode ? `
      /* Screen-only micro adjustments for tight containers (does NOT affect print) */
      @media screen {
        body { max-width: 210mm; }
        .container { padding: 12mm; }
        .section-title { font-size: 13px; }
        .job-title { font-size: 12.5px; }
        .company { font-size: 11.5px; }
        .responsibilities li { font-size: 10.5px; }
      }
    ` : ''}
  `;
