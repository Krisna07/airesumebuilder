  ...
  return (
    <>
        ...
        <div key={index} className='w-full grid grid-cols-1 gap-4 place-items-start'>
          <div className='w-full grid gap-2 relative'>
            <div className='flex items-center justify-between gap-4'>
              <Input type='text' name='title' required value={experience.title} onChange={(e) => updateExperience(index, 'title', e.target.value)} placeholder='Title' />
              <button type='button' onClick={() => removeExperience(index)} className='text-red-500 hover:text-red-600'>
                <Trash2 size={20} />
              </button>
            </div>

            <Input type='text' name='company' required value={experience.company} onChange={(e) => updateExperience(index, 'company', e.target.value)} placeholder='Company' />
            <Input type='text' name='location' required value={experience.location} onChange={(e) => updateExperience(index, 'location', e.target.value)} placeholder='Location' />
            <div className='w-full flex gap-2 items-center justify-between'>
              <Datepicker index={index} target='startDate' value={experience.startDate ? experience.startDate : ''} update={updateExperience} />
              <Datepicker index={index} current={experience.current} target='endDate' value={experience.endDate ? experience.endDate : ''} update={updateExperience} />
            </div>
            <label className='w-fit flex items-start cursor-pointer'>
              <input type='checkbox' name='current' checked={experience.current} className='sr-only peer' onChange={(e) => toggleCurrentEmployment(index, e.target.checked)} />
            ...
            >
              ...
              <div className='flex gap-2 items-center'>
                ...
                />
                <button type='button' onClick={() => addResponsibility(index)} className='text-green-600 font-semibold'>
                  Add
                </button>
              </div>
            </form>
      ...
      <div className='w-full flex gap-4 items-start p-2'>
        <Button type='submit' variant='secondary' size='small' onClick={addExperience}>
          Add Experience
        </Button>
      </div>
    </>
