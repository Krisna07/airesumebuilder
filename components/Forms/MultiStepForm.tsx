...
interface MultiStepFormProps {
  ...
  onSave: (data: ResumeData, description?: string) => Promise<string | null>;
  isSaving: boolean;
  isDirty: boolean;
...
const MultiStepForm: React.FC<MultiStepFormProps> = ({ 
  ...
  useEffect(() => {
    handleResumeDataUpdate(formData);
  }, [formData, handleResumeDataUpdate]);
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 6) {
      const success = await onSave(formData, jobDescription);
      if (success) {
  ...
  const handleSaveDraft = async () => {
    await onSave(formData, jobDescription);
  };
            ...
            <div className="mt-6 gap-4 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  size="small"
                  onClick={handleNext}
                  disabled={currentStep === 7 || (currentStep === 6 && isSaving)}
                >
                  {isSaving && currentStep === 6 
                    ? "Saving..." 
                    : currentStep === 6 
                      ? "Complete & Save" 
                      : "Next"
