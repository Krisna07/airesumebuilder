...
const RecommendationUI: React.FC<RecommendationUIProps> = ({
  ...
  // Auto-fetch/refetch when core parameters change
  useEffect(() => {
    setHasAutoFetched(false);
    fetchRecommendations();
  }, [title, specialization, seniority]);

  // Auto-trigger AI if local results are thin
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* List Content */}
      {loading ? (
        <div className="flex items-center justify-center p-4"><Loader2 className="w-5 h-5 animate-spin text-blue-500" /></div>
      ) : (
