...
function extractProfile(text: string): Partial<Profile> {
  ...
  if (lines.length > 0) {
    ...
    if (!SECTION_PATTERNS.summary.keywords.some((k) => firstLine.toLowerCase().includes(k))) {
...
export function extractResumeFromText(
  ...
  const headerEndIdx = sections.size > 0 ? Math.min(...Array.from(sections.values()).map((s) => s.start)) : text.indexOf(text.split('\n')[5] || '');
  const headerText = text.substring(0, Math.max(headerEndIdx, 200));
  const profile = extractProfile(headerText);

  // Extract summary from section if present
  if (sections.has('summary')) {
    const { start, end } = sections.get('summary')!;
    const summaryText = extractSectionText(text, start, end);
  ...
  if (sections.has('skills')) {
    ...
    const skillsText = extractSectionText(text, start, end);
  ...
  if (sections.has('experience')) {
    ...
    const experienceText = extractSectionText(text, start, end);
  ...
  if (sections.has('education')) {
    ...
    const educationText = extractSectionText(text, start, end);
    educations = extractEducations(educationText);
  }

  // Extract custom sections
  const customSections = extractCustomSections(text, sections);
